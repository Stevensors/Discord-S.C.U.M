...
class SuperProperties:
    ...
    def requestBuildNumber(self):
        ...
        try:  # getting the build num is kinda experimental since who knows if discord will change where the build number is located...
            extraMods = {
                "update": {"Sec-Fetch-Dest": "document", "Sec-Fetch-Mode": "navigate", "Sec-Fetch-Site": "none"}}

            res = Wrapper.sendRequest(self.editedS, 'get', "https://discord.com/login", headerModifications=extraMods,
                                      log=False)
            if res:
            ...
            file_with_build_num = 'https://discord.com/assets/' + \
                                  re.compile(r'assets/+([a-z0-9]+)\.js').findall(discord_login_page_exploration)[
                                      -2] + '.js'
            req_file_build = Wrapper.sendRequest(self.editedS, 'get', file_with_build_num,
    ...
    def getSuperProperties(self, user_agent, locale):
        ...
        browser_ver_list = [parseduseragent["user_agent"]["major"], parseduseragent["user_agent"]["minor"],
                            parseduseragent["user_agent"]["patch"]]
